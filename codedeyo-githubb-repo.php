...
add_filter('plugin_action_links_'.plugin_basename(__FILE__), 'githubb_repo_link');
function githubb_repo_link( $links ) {
  $links[] = '<a href="' .
 ...
 add_action( 'admin_init', 'git_update_user_settings' );

   if( !function_exists("git_update_user_settings") ) { 
    function git_update_user_settings() {  
  ...
  add_action( 'wp_footer', 'load_codedeyo_js', 100);
	...
	function codedeyo_githubb_repo_page(){
		...
		<div class="wrap">
          <div class="mainrepo2">
            <center>
              <h2 class="githeaderr"><img src="<?php echo plugin_dir_url( __FILE__ ).'assets/'; ?>loader/icon.png" height="17px"> Codedeyo GitHubb Repositories</h2>
      ...
      <form method="post" action="options.php">     
        ...
        <?php do_settings_sections( 'git-user-settings-codedeyo' ); ?>
              <p>
                Enter GitHubb Username: <input id="gitbasevalue" type="text" name="gituser_username" value="<?php echo str_replace(' ', '',get_option( 'gituser_username' )); ?>" placeholder="e.g adeleyeayodeji">
              </p>     
        <?php submit_button(); ?>   
      </form>  
      </div>
